...
file = "HEFT_reweighting_GluGlutoHHto2B2Tau_kl_1p00_kt_1p00_c2_0p35.json"
...
df = df.Define("w_nominal", f'nloweight / {NtotalEvents}')
df = df.Define("w_reweight", f'nloweight*GetWeight(mhh_gen, pthh_gen, costhetastar_gen, "{file}") / {NtotalEvents}')
# df = df.Define("w_reweight", f'GetWeight(mhh_gen, pthh_gen, costhetastar_gen, "{file}")')


output_file = ROOT.TFile("example.root", "RECREATE")
...
output_file.Close()
print("Histograms saved to example.root")